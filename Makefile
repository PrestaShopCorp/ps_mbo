...
ifndef DOCKER
	...
	docker run --rm -d -e PS_DOMAIN=localhost -e PS_ENABLE_SSL=0 -e PS_DEV_MODE=1 -e PS_VERSION=8.0 --name test-phpunit prestashop/docker-internal-images:nightly
	...
	docker container exec -u www-data test-phpunit sh -c "sleep 1 && ls -al /var/www/html/modules/ps_mbo/var/ && php -d memory_limit=-1 ./bin/console prestashop:module install ps_mbo"
